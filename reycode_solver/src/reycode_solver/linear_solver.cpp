...
namespace reycode {
    class AMGCL_Solver : public Linear_Solver<Matrix<real,uint32_t,Kokkos::HostSpace>> {
        ...
        >;
    public:
        using Matrix = reycode::Matrix<real,uint32_t,Kokkos::HostSpace>;
    ...
    void AMGCL_Solver::solve(const Matrix& matrix, Kokkos::View<real*,Kokkos::HostSpace> x, Kokkos::View<real*,
        ...
        Solver solver(std::tuple(n, range(matrix.rowBegin), range(matrix.cols), range(matrix.coeffs)));
